  ...
  return (
    <div className="bg-white text-gray-900 min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-6 py-10">
        {/* 게시글 제목 */}
        <div
          className={`mx-5 text-4xl font-bold mb-6 tracking-wide ${'text-black'}`}
        >
          {board.title}
        </div>
        ...
        <div className="mx-5 flex justify-between items-center mb-6">
          <span className={`text-sm font-GowunBatang ${'text-black'}`}>
          ...
          {localStorage.getItem('nickname') === board.authorNickName && (
            <div className="flex gap-2">
              <button
                onClick={() => router.push(`/board/update/${code}`)}
        ...
        {/* 게시글 내용 */}
        <div
          className={`mx-16 min-h-[200px] rounded-xl ${'bg-white-50 '}`}
        ...
        <div
          className={`border-t my-6 w-1/2 mx-auto ${'border-gray-600'}`}
        ></div>
        ...
        <section className="mt-10 mx-14">
          ...
          <div className="flex gap-2 mb-4">
            <input
              ...
              className={`flex-1 border rounded px-3 py-2 ${'bg-white text-gray-900 border-gray-300'}`}
            />
            <button
          ...
          <ul className="space-y-3">
            {comments.map((c) => (
              <li
                key={c.code}
                className={`border rounded px-4 py-2 ${'border-gray-300 bg-white'}`}
              >
