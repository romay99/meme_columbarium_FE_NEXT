    ...
    <div className={`${"bg-white text-black"} min-h-screen transition-colors duration-500`}>
      ...
      <div className="max-w-4xl mx-auto p-6 flex flex-col space-y-4">
        ...
        <div className="flex flex-col">
          ...
          <input className={`border rounded p-2 w-full ${"bg-white text-black border-gray-300"}`} placeholder="제목" value={title} onChange={(e) => setTitle(e.target.value)} />
        </div>
        ...
        <div className="flex gap-4">
          <div className="flex flex-col w-1/2">
            ...
            <input type="month" className={`border rounded p-2 w-full ${"bg-white text-black border-gray-300"}`} value={startDate} onChange={(e) => setStartDate(e.target.value)} />
          </div>
          <div className="flex flex-col w-1/2">
            <label className="mb-1 font-semibold">
              밈 망한 날짜<span className="text-red-500"> *</span>
            </label>
            <input type="month" className={`border rounded p-2 w-full ${"bg-white text-black border-gray-300"}`} value={endDate} onChange={(e) => setEndDate(e.target.value)} />
          </div>
        </div>
        ...
        <div className="flex flex-col">
          ...
          <select className={`border rounded p-2 w-full ${"bg-white text-black border-gray-300"}`} value={category} onChange={(e) => setCategory(e.target.value)}>
            {categories.map((cat) => (
        ...
        {/* 마크다운 에디터 */}
        <div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} className="border rounded p-2" data-color-mode={"light"}>
          <MDEditor
            value={contents}
        ...
        <div className="flex justify-end">
          <button className={`w-35 rounded-lg px-4 py-2 transition ${"bg-gray-500 hover:bg-gray-400 text-white"}`} onClick={handleSubmit}>
            작성 완료
          </button>
        </div>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 font-GowunBatang">
          <div className={`p-6 rounded-lg shadow-lg max-w-lg w-full ${"bg-white text-black"}`}>
            ...
            <div className="flex justify-end gap-4">
              <button className={`px-4 py-2 rounded transition ${"bg-gray-300 hover:bg-gray-400 text-black"}`} onClick={() => router.push("/meme/list")}>
                아니오
              </button>
              <button className={`px-4 py-2 rounded transition ${"bg-blue-500 hover:bg-blue-400 text-white"}`} onClick={() => setShowModal(false)}>
                예
              </button>
            </div>
          </div>
