...
function MemeUpdatePage() {
  ...
  const fetchMemeDetail = async () => {
    try {
      ...
      const response = await api.get(`${serverUrl}/meme/info?code=${code}`, {
        headers,
      });
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div
            className={
              darkMode
                ? 'bg-gray-800 text-white rounded-lg shadow-lg z-10 w-96 p-6 flex flex-col space-y-4 font-GowunBatang'
                : 'bg-white text-black rounded-lg shadow-lg z-10 w-96 p-6 flex flex-col space-y-4 font-GowunBatang'
            }
          >
            <h2 className="text-lg font-bold text-center">수정 유의사항</h2>
            <ul className="list-disc list-inside space-y-2">
              <li>
                이 밈 게시물은 수정 내역이 기록됩니다. 신중하게 편집해주세요.
              </li>
              <li>타인의 게시물은 존중하며, 허락 없이 변경하지 말아주세요.</li>
            </ul>
            <p className="text-sm text-center">
              아래 버튼으로 동의 여부를 선택해주세요.
            </p>
            <div className="flex justify-center gap-4 mt-2">
              ...
              <button
                className="bg-red-500 text-white rounded-lg px-4 py-2 hover:bg-red-400 transition"
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div
            className={
              darkMode
                ? 'bg-gray-800 text-white rounded-lg shadow-lg z-10 w-96 p-6 flex flex-col space-y-4'
                : 'bg-white text-black rounded-lg shadow-lg z-10 w-96 p-6 flex flex-col space-y-4'
            }
          >
