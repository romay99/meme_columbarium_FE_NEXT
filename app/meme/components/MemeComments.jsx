...
const MemeComments = (props) => {
  ...
  const checkLikeButton = async () => {
    ...
    await api
      .get(`${serverUrl}/likes/check?orgMemeCode=${props.orgMemeCode}`, {
        headers: { Authorization: token },
      })
      .then((res) => {
        if (res.data) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap sm:flex-nowrap justify-end gap-2 sm:gap-3 mt-4">
        <button
          onClick={() => router.push(`/meme/update/${props.memeCode}`)}
          className={`px-3 sm:px-4 py-2 rounded ${'bg-green-300 hover:bg-green-400 text-gray-900'}`}
        >
          밈 수정하기
        </button>
        <button
          onClick={() =>
            router.push(`/meme/update-history/${props.orgMemeCode}`)
          }
          className={`px-3 sm:px-4 py-2 rounded ${'bg-blue-300 hover:bg-blue-400 text-gray-900'}`}
        >
          수정기록 열람
        </button>
      </div>
      ...
      <section className="mt-6 sm:mt-10 mx-2 sm:mx-4 md:mx-14">
        ...
        <div className="flex flex-col sm:flex-row gap-2 mt-2 mb-4">
          <input
            ...
            className={`flex-1 border rounded px-2 sm:px-3 py-2 ${'bg-gray-200 text-gray-900 placeholder-gray-500'}`}
          />
          <button
            onClick={handleAddComment}
            className={`px-3 sm:px-4 py-2 rounded ${'bg-gray-400 hover:bg-gray-300 text-gray-900'}`}
          >
            등록
          </button>
        </div>
        ...
        <ul className="space-y-2 sm:space-y-3">
          {comments.map((c) => (
            <li
              key={c.code}
              className={`border rounded px-3 sm:px-4 py-2 ${'bg-white text-gray-900 border-gray-300'}`}
            >
      ...
      <section className="flex justify-end mt-4 sm:mt-6 mx-2 sm:mx-6 md:mx-14">
        <button
          ...
          className={`px-4 py-2 rounded ${'bg-gray-300 hover:bg-blue-400 text-gray-900'}`}
        >
          목록
        </button>
      </section>
